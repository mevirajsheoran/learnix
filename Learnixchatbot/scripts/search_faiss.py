...
def load_faiss_data() -> Tuple[Optional[faiss.Index], List[dict]]:
    ...
    # print(f"Looking for FAISS index at: {VECTOR_STORE_PATH}")
    # print(f"Exists: {os.path.exists(VECTOR_STORE_PATH)}")
...
def search_faiss(query: str, top_k: int = TOP_K) -> List[dict]:
    ...
    if index is None or not chunks:
        print("❌ FAISS index or chunks not found!")
        return []

    print(f"✅ FAISS index and {len(chunks)} chunks loaded successfully.")

    query_embedding = get_openai_embedding(query)
    if query_embedding is None:
        print("⚠️ Failed to generate OpenAI embedding for query.")
        return []
    ...
    try:
        ...
        results = []
        print(f"📊 FAISS returned {len(indices[0])} results (before filtering).")
        ...
        for i, (dist, idx) in enumerate(zip(distances[0], indices[0])):
            if 0 <= idx < len(chunks) and dist >= SIMILARITY_THRESHOLD:
                ...
                print(f"Match {i+1}: Page {result['page_number']} | Similarity: {result['similarity']}")

        if not results:
            print("⚠️ No results passed similarity threshold.")
...
def generate_cohesive_answer(query: str, results: List[dict]) -> str:
    """Generate a response using retrieved context, fallback knowledge, or 'out of scope'."""
    
    logging.info(f"📩 Processing query: '{query}'")

    if results:
        logging.info(f"✅ FAISS returned {len(results)} relevant results.")
        context = "\n\n".join([f"Page {res['page_number']}: {res['text']}" for res in results])
        ...
        logging.info(f"📜 Generated OpenAI prompt (trimmed): {response_prompt[:200]}...")  # Logs first 200 chars

        try:
            response = client.chat.completions.create(
                model=GPT_MODEL,
                messages=[
                    {"role": "system", "content": "You are a science assistant that strictly answers based on the provided textbook excerpts. Do not generate any information beyond the retrieved context."},
            ...
            logging.info(f"💡 OpenAI Response: {answer[:200]}...")  

         
            if "out of my scope" in answer.lower():
                logging.warning("⚠️ Response is out of scope.")
    ...
    logging.warning(f"⚠️ No relevant FAISS results found for query: '{query}'")

    query_lower = query.lower().strip()

    for key, fallback_answer in FALLBACK_ANSWERS.items():
        if key.lower() in query_lower:
            logging.info(f"✅ Using fallback answer for '{key}'")
            return fallback_answer

    logging.warning("⚠️ No fallback answer found. Returning 'out of scope'.")
    return "This is out of my scope."
